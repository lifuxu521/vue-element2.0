   ...
   <section>
     <div class="page-funcarea clearfix">
      ...
      <div class="pageFuncarea-btn clearfix fl">
          <el-button type="success" @click="getOpenCardList()">查询</el-button>
      ...
      <div class="page-table-box">
          ...
          <template>
            <el-table  :data="tableData"    style="width: 100%">
              <el-table-column fixed prop="ic_no"  align="center" label="卡号" >
              </el-table-column>
...
<script>
import {getOpenCardList} from '../../axios/axios';
    ...
    cert_type(row,column,cellValue){
     ...
     handleSizeChange(val) {
      ...
      handleCurrentChange(val) {
         let _this = this;
        let page_num = val*1;
        _this.getOpenCardList(page_num,_this.searchinfo.page_size);
      },
      resSetSearch(){
        this.time = "";
        this.searchinfo.ic_no = "";
     },
      getOpenCardList(page_num,page_size){
        let _this = this;
        let time = this.time;
        let params = {
        ...
        getOpenCardList(params).then((res) => {
          console.log(res)
           if(res.code=="200000"){
                if(page_size=="-1"||page_size==-1){
                    ...
                    res.data.map((item) => {
                        ...
                        item.state = util.userstate(item.state);
                        return item;
                    });
                    _this.list = res.data;
                }else{
      ...
      excel_down(){
          ...
          import('@/vendor/Export2Excel').then(excel => {
            const tHeader = [ '卡号', '姓名', '证件类型', '证件号','状态','发卡日期','更新日期']
            const filterVal = ['ic_no', 'user_name', 'cert_type', 'cert_num','state','open_time','update_time']

    ...
    <el-col :span="8" :offset="1" class="el_row">
      <ul class="login-box-modify">
        ...
        <li class="ipt-item">
          <div class="ipt-form">
            <span>新密码：</span>
            <input type="password" v-model="loginToinfo.newPass" placeholder="新密码长度为6位数字">
          </div>
        </li>
        <li class="ipt-item">
          <div class="ipt-form">
...
<script>
  ...
  import {modifyBusiChgpwd} from '../../axios/axios';
  export default{
    data(){
      return {
        loginToinfo: {
    ...
    methods: {
      ...
      modify_save(){
        ...
        let params = {
          ...
          oper_code:this.$store.state.loginInfo.ic_no
        };
        modifyBusiChgpwd(params).then(res=>{
          if(res.code == 200000){
            this.$message({
              message: res.msg,
              type: 'success',

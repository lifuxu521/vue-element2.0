  ...
  <section>
    <div class="pageinfo-top">
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <div class="info-top-lf"><i class="info-icon info-yetcharge" :style="{backgroundImage: 'url(' + icon + ')'}"></i></div>
            <div class="info-top-rt">
        ...
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <div class="info-top-lf"><i class="info-icon info-yesbusiness" :style="{backgroundImage: 'url(' + icon + ')'}"></i></div>
            <div class="info-top-rt">
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <div class="info-top-lf"><i class="info-icon info-chong"
                                        :style="{backgroundImage: 'url(' + icon + ')'}"></i></div>
            <div class="info-top-rt">
              <span class="infotext-top">昨日充值金额</span>
              <span class="infotext-center">￥{{query_day_recharge_amount}}</span>
              <span class="infotext-bottom">本月充值金额：￥{{query_month_recharge_amount}}</span>
            </div>
          </div>
        </el-col>
        <el-col :span="8">
          <div class="grid-content bg-purple">
            <div class="info-top-lf"><i class="info-icon info-tui" :style="{backgroundImage: 'url(' + icon + ')'}"></i>
            </div>
            <div class="info-top-rt">
              <span class="infotext-top">昨日退卡数量</span>
              <span class="infotext-center">{{query_day_cancel_amount}}张</span>
              <span class="infotext-bottom">本月退卡数量：{{query_month_cancel_amount}}</span>
            </div>
          </div>
        </el-col>
      </el-row>
    ...
    <div class="pageinfo-chart">
      <div v-show="tabtoggleNum == 0" :style="{height: 336+'px'}">
        <div style="text-align: left" class="page_nav">
          <el-tooltip class="item" effect="dark" placement="bottom-start" :popper-class="toolTipClass">
          ...
          <el-date-picker
            ...
            format="yyyy-MM-dd" value-format="yyyyMMdd" @change="changeTime()" >
          </el-date-picker>
          &nbsp&nbsp&nbsp&nbsp
          <span>机构查询：</span>
          <el-select v-model="searchinfo.group_id" filterable placeholder="请选择所属机构" @change="EchartsprevData(false)">
            <el-option
      ...
      <!--<div v-show="tabtoggleNum == 1">2</div>-->
      <!--<div v-show="tabtoggleNum == 2">3</div>-->
    </div>
...
<script>
  ...
  import {requestDayData,requestMonthData,getGroupList,requestTotalDayData,requestTotalMonthData} from '../../axios/axios';
  ...
  export default {
    ...
    data () {
      return {
        ...
        query_day_recharge_amount: '',
        query_day_cancel_amount: '',
        query_month_open_amount: '',
        query_month_pay_amount: '',
        query_month_busi_amount: '',
        query_month_recharge_amount: '',
        query_month_cancel_amount: '',
    ...
    methods: {
      ...
      getprevData(){
        ...
        requestTotalDayData(params1).then(res => {
        ...
        requestTotalDayData(params2).then(res => {
           console.log(res);
        ...
        requestTotalDayData(params3).then(res => {
//            console.log(res)
        ...
        requestTotalDayData(params4).then(res => {
//            console.log(res)
          this.query_day_recharge_amount = util.toDecimal(res.data?res.data[0].amount:0)
        });
        let params5 = {
          "busi_type": "cancel",
          "start_day": preDate,
        ...
        requestTotalDayData(params5).then(res => { 
          this.query_day_cancel_amount = util.oparseInt(res.data?res.data[0].amount:0)
        });
      },
      getprevMonth(){
        let curDate = new Date();
        ...
        if (thisDate == 1) {
          ...
          requestTotalMonthData(params1).then(res => {
          ...
          requestTotalMonthData(params2).then(res => {
          ...
          requestTotalMonthData(params3).then(res => {
          ...
          requestTotalMonthData(params4).then(res => {
            this.query_month_recharge_amount = util.toDecimal(res.data?res.data[0].amount:0);
          });
          let params5 = {
            "busi_type": "cancel",
            "start_month":preMonth,
          ...
          requestTotalMonthData(params5).then(res => {
            this.query_month_cancel_amount = util.oparseInt(res.data?res.data[0].amount:0);
          });
        }
      ...
      changeTime(){
       this.timePicker=1;
       this.EchartsprevData(false);
      },
      EchartsprevData(flag){
        this.EchartsList = [];
        this.xAxis = [];
        let start_day = "";
        let end_day = "";
        ...
        if(this.searchinfo.group_id == 'ALL'){
                let params1 = {
                  "busi_type": "open",
                  "start_day": start_day,
                  "end_day": end_day
        ...
        let group_id = this.searchinfo.group_id;
        let params1 = {
          'group_id': group_id,
          "busi_type": "open",
          "start_day": start_day,
  ...
  //            console.log(i)
      ...
      getGroupList(page_num, page_size){
        ...
        getGroupList(params).then((res) => {
          ...
          if (res.code == "200000") {
             this.groupOptions = [];
             res.data.unshift({'group_id':"ALL",'group_name':'全部机构'});
             this.groupOptions = res.data;
             this.searchinfo.group_id = 'ALL';
              this.EchartsprevData(true);
          }
          ;
        });
      },
